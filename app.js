...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cell.style.cssText = [
      ...
      'gap:3px',
      'cursor:pointer',
      'border:1px solid ' + (isSelected ? 'var(--gold)' : isToday ? 'rgba(201,168,76,.4)' : 'rgba(201,168,76,.08)'),
      'background:' + (isSelected ? 'rgba(201,168,76,.15)' : hasData ? 'rgba(201,168,76,' + intensity * 0.18 + ')' : 'rgba(13,11,9,.3)'),
      'transition:all .15s',
      'position:relative'
    ].join(';');

    const numEl = document.createElement('div');
    numEl.style.cssText = 'font-family:\'Cinzel\',serif;font-size:10px;color:' + (isSelected ? 'var(--gold)' : isToday ? 'var(--gold-light)' : hasData ? 'var(--parchment)' : 'var(--fog)') + ';line-height:1';
    numEl.textContent = d;
    cell.appendChild(numEl);

    if (hasData) {
      const dot = document.createElement('div');
      dot.style.cssText = 'font-family:\'Cinzel\',serif;font-size:7px;color:' + (isSelected ? 'var(--gold)' : 'rgba(201,168,76,.6)') + ';letter-spacing:0';
      ...
      fill.style.cssText = 'height:100%;width:' + pct + '%;background:' + (pct >= 80 ? 'var(--gold)' : pct >= 50 ? 'var(--ember)' : 'var(--blood)') + ';transition:width .4s';
    ...
    cell.addEventListener('mouseleave', () => {
      if (key !== calSelectedDate) cell.style.borderColor = isToday ? 'rgba(201,168,76,.4)' : 'rgba(201,168,76,.08)';
    });
...
function renderDayDetail(key) {
  ...
  document.getElementById('chart-summary').textContent = total > 0 ? total + '問 学習済み · 正解 ' + correct + ' · 不正解 ' + wrong : 'この日の学習記録はありません';
  ...
  if (total > 0) {
    ...
    dc.style.strokeDashoffset = circ - correctArc;
    dc.style.strokeDasharray = correctArc + ' ' + (circ - correctArc);
    dw.style.strokeDashoffset = -correctArc;
    dw.style.strokeDasharray = wrongArc + ' ' + (circ - wrongArc);
  } else {
    document.getElementById('donut-pct').textContent = '—';
    document.getElementById('donut-correct').style.strokeDashoffset = circ;
    document.getElementById('donut-wrong').style.strokeDashoffset = circ;
  }
  ...
  hourly.forEach((count, hour) => {
    ...
    bar.style.cssText = [
      'width:100%','height:0',
      'transition:height .6s ease ' + (hour * 18) + 'ms',
